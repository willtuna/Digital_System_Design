...
as pressed is released, row start shifting ........
  ...
  ******************************************************/
module Keypad_Top(clk, rst, col_in, row_out, enc_out);
		input clk, rst;
		input [3:0] col_in;
		output [3:0] row_out;
		output [7:0] enc_out;
		...
		Column_Row_Shifter keypad_sweep(keypad_clk,rst,col_in,row_out,keypad_code);
...
module Column_Row_Shifter ( clk, rst, in, row_sweep,press_pos );
		input clk, rst;
		input [3:0] in;
...
//   This is for press detection
		always@(*)
				if(in == col_current) press_pos =  rst ? {8{1'b1}}  : {in,row_current};
